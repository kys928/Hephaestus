...
Three material cross-branch gaps were found:
...
3. the existing autonomous coordinator still bound fixture-era keys (`data_contract_ref` and `tokenizer_ref`) while the strict Transformers lifecycle requires persisted trainable-data-contract, processed-data, processing-evidence, tokenizer-revision, optimizer, scheduler, training-mode, and content-hash fields.

The integration branch adds the missing generation bridge, `training`, `generation`, and combined `ml` extras, and `ProductionAutonomyCoordinator` for backend-correct real-training input binding while preserving the existing fixture coordinator path.

## Real training composition

`hephaestus.control.ProductionAutonomyCoordinator` subclasses the reviewed coordinator and changes only the training-binding seam when `backend_id="transformers_causal_lm"`.

It binds and hashes:

- persisted `trainable_data_contract.json`;
- processed JSONL;
- processing evidence;
- model and tokenizer identities and revisions;
- architecture, parameter count, context length, hidden size, vocabulary size, and special-token evidence;
- training mode, optimizer, scheduler, step budget, learning rate, device, and dtype;
- model license, provenance, and approval references.

Missing persisted data artifacts fail before launch. The original local-fixture binding remains unchanged for existing callers.
...
- rejects empty outputs and unsupported finish states;
...
`DeterministicFakeGenerationBackend` provides bounded offline tests. `TransformersCausalLMGenerationBackend` lazily imports optional dependencies, requires model and tokenizer artifacts from the same finalized checkpoint, independently verifies the checkpoint manifest, disables remote code, and performs no implicit network acquisition.
...
Every required task/seed pair must exist before a report is complete. Backend failures, empty output, invalid finish states, or missing samples produce partial evidence and blocking issues rather than false completion.
...
- The staged orchestrator is opt-in. A deployment composition root must still provide concrete diagnosis, provider, selection, acquisition, preprocessing, model selection, lifecycle, runtime, recovery, Judge, approval, and action adapters. The reviewed coordinator, real-training binding, generation adapter, and evaluation adapter now provide the core experiment path but do not invent deployment credentials or operator policy.
