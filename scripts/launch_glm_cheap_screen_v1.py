...
d={
    ...
    'nvidia_smi_returncode':int(p.returncode),
    'nvidia_smi_stdout':p.stdout.strip(),
    'nvidia_smi_stderr':p.stderr.strip(),
}
print('GLM_CHEAP_SCREEN_CUDA_JSON '+json.dumps(d,sort_keys=True),flush=True)
if not d['cuda_available'] or d['device_count'] != 1:
    raise SystemExit('GLM cheap-screen CUDA preflight failed')
