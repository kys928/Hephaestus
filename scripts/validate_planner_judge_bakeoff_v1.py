...
def main():
    ...
    need(c["governance"]["paid_launch_allowed"] is False,"bakeoff must remain unlaunched")
    marker=json.loads((ROOT/"configs/experiments/planner_judge_bakeoff_v1.launch.json").read_text())
    need(marker["authorized"] is False,"launch marker must remain unauthorized")
    print(json.dumps({"status":"valid","pack_sha256":observed,"planner_cases":{k:len(v) for k,v in p["partitions"]["planner"].items()},"judge_cases":{k:len(v) for k,v in p["partitions"]["judge"].items()},"candidate_role_pairs":8,"paid_launch_allowed":False},sort_keys=True))
