#!/usr/bin/env python3
"""Render or launch the bounded paired diagnosis adaptability experiment."""
...
print('DIAG_ADAPT_GPU_JSON '+json.dumps({'returncode':p.returncode,'stdout':p.stdout.strip(),'stderr':p.stderr.strip()},sort_keys=True),flush=True)
...
print('DIAG_ADAPT_CUDA_JSON '+json.dumps(d,sort_keys=True),flush=True)
if not d['available'] or d['count']!=1:raise SystemExit('CUDA preflight failed')
x=torch.ones(1,device='cuda');torch.cuda.synchronize();print('DIAG_ADAPT_CUDA_OK '+str(float(x.item())),flush=True)
PYCUDA
"$PY" scripts/validate_diagnosis_adaptability_v1.py
"$PY" -m py_compile scripts/run_diagnosis_adaptability_v1.py
"$PY" scripts/run_diagnosis_adaptability_v1.py
'''
def env(repo_sha,run_id,real=False):
    d={"HEPHAESTUS_REPO_SHA":repo_sha,"HEPHAESTUS_DIAG_ADAPT_RUN_ID":run_id,"RUNPOD_S3_ACCESS_KEY_ID":"<secret>","RUNPOD_S3_SECRET_ACCESS_KEY":"<secret>","RUNPOD_S3_ENDPOINT_URL":"<endpoint>","RUNPOD_DATACENTER_ID":"<region>","RUNPOD_NETWORK_VOLUME_ID":"<bucket>","PYTHONUNBUFFERED":"1"}
    if real:
...
def body(c,name,e):
    x=c["execution"]
    b={"name":name[:180],"computeType":"GPU","gpuCount":1,"gpuTypeIds":x["gpu_type_ids"],"gpuTypePriority":"availability","cloudType":x["cloud_type"],"imageName":x["image"],"containerDiskInGb":x["container_disk_gb"],"dockerStartCmd":["bash","-lc",shell()],"interruptible":False,"env":e}
...
def execute(c):
    ...
    try:
        ...
        while True:
            ...
            if elapsed>=float(c["execution"]["hard_wall_seconds"]): raise TimeoutError("adaptability hard wall reached")
        ...
        if pod_id:
            td=lifecycle.delete_pod_with_retries(ex,pod_id);print("DIAG_ADAPT_TEARDOWN_JSON "+json.dumps(td,sort_keys=True),flush=True)
