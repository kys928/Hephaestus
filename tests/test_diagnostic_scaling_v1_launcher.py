...
def test_all_rendered_requests_are_ephemeral_global_and_immutable():
    payload = contract()
    for candidate in payload["candidates"]:
        ...
        assert rendered["placement_scope"] == "global_no_datacenter_filter"
        request = rendered["request"]
        assert "networkVolumeId" not in request
        assert "volumeMountPath" not in request
        assert "dataCenterIds" not in request
        assert "countryCodes" not in request
...
def test_large_moe_candidates_allow_h200_then_b200():
    payload = contract()
    for candidate in payload["candidates"]:
        ids = launcher.gpu_ids(payload, candidate)
        if candidate["minimum_gpu_memory_gib"] >= 140:
            assert ids == ["NVIDIA H200", "NVIDIA B200"]
        else:
            assert "NVIDIA H200" in ids
            assert "NVIDIA B200" in ids
            assert "NVIDIA RTX PRO 6000 Blackwell Server Edition" in ids
            assert any("H100" in value or "A100" in value for value in ids)


def test_location_filter_injection_is_rejected():
    payload = contract()
    ...
    request["dataCenterIds"] = ["EU-CZ-1"]
    with pytest.raises(ValueError, match="location filter"):
