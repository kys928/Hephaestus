...
def test_repository_preflight_is_explicitly_not_launchable(monkeypatch):
    ...
    assert summary["status"] == "recovery_inputs_validated_not_launchable"
    assert summary["paid_launch_allowed_now"] is False
    assert summary["paid_launch_admission_persisted"] is False
    assert "admission.json" not in objects


def test_persist_launch_admission_is_blocked_before_new_user_go(monkeypatch):
